...
* The final classic RPG architecture target is now mapped in `docs/architecture/classic-rpg-architecture-plan.md`, with scene/state/package boundaries and AI work boundaries recorded before expanding beyond the Field spike.
* Future implementation should follow the scene docs under `docs/scenes/`, runtime docs under `docs/runtime/`, and dependency boundaries in `docs/architecture/dependency-boundaries.md`; do not add more command effects without command sequencing/runtime-state design.
* Architecture boundary skeleton directories now exist for future `runtime-core`, `runtime-systems`, `validation`, `editor-model`, `project-schema`, and `event-commands` packages. Future runtime work should target `runtime-core`, RPG rule work should target `runtime-systems`, diagnostics should target `validation`, and editor-only state should target `editor-model`; existing implementation has not been moved yet.
* A Godot Battle Scene Spike is now designed in `docs/godot-battle-spike.md` as a fixed-data playable-feel experiment. It does not replace runtime-core battle architecture and should avoid schema, SaveData, inventory, rewards, audio, and field connection changes in the first step.
