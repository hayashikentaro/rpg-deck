...
* The Godot debug grid can now detect `trigger: interact` events in the player-facing cell with Enter, Space, or Z and logs the event id only. Touch events, dialogue UI, choices, flags, transfer, battle/audio, and command execution remain out of scope.
